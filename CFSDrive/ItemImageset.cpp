...
HRESULT CFlickrItemImageset::GetExtractIcon(REFIID riid, LPVOID* ppRetVal)
{
   HR( _EnsureFlickrRef() );
   HRESULT Hr = E_FAIL;
   if( m_pImageset->Type == FLICKRFS_IMAGESET_PHOTOSET ) 
      Hr = ::SHCreateExtractIconFromKnownFolder(FOLDERID_Pictures, riid, ppRetVal);
   if( m_pImageset->Type == FLICKRFS_IMAGESET_FAVOURITES || m_pImageset->Type == FLICKRFS_IMAGESET_PUBLIC_FAVOURITES ) 
      Hr = ::SHCreateExtractIconFromKnownFolder(FOLDERID_Favorites, riid, ppRetVal);
   if( m_pImageset->Type == FLICKRFS_IMAGESET_TAGSEARCH ) 
      Hr = ::SHCreateExtractIconFromKnownFolder(FOLDERID_SavedSearches, riid, ppRetVal);
   if( m_pImageset->Type == FLICKRFS_IMAGESET_RECENT ) 
      Hr = ::SHCreateExtractIconFromKnownFolder(FOLDERID_Downloads, riid, ppRetVal);
   if( SUCCEEDED(Hr) ) 
      return Hr;
...
HRESULT CFlickrItemImageset::GetPaneState(REFEXPLORERPANE ep, EXPLORERPANESTATE* peps)
{
   if( ep == EP_DetailsPane ) 
      *peps = EPS_DEFAULT_ON | EPS_INITIALSTATE;
   return S_OK;
}
...
HRESULT CFlickrItemImageset::GetProperty(REFPROPERTYKEY pkey, CComPropVariant& v)
{
   ...
   if( pkey == PKEY_Title ) {
      if( m_pImageset->Type != FLICKRFS_IMAGESET_PHOTOSET && m_pImageset->Type != FLICKRFS_IMAGESET_TAGSEARCH ) 
         return E_FAIL;
      return ::InitPropVariantFromString(m_pImageset->sTitle, &v);
   }
...
{
   HR( _EnsureFlickrRef() );
   HR( _FlickrAPI.SetImagesetProperty(m_pImageset, pkey, v) );
...
{
   if( !CPidlMemPtr<FLICKRFS_PIDL_PHOTO>(pidlItem).IsType(FLICKRFS_MAGIC_ID_PHOTO) ) 
      return NULL;
...
HRESULT CFlickrItemImageset::GetChild(LPCWSTR pwstrName, SHGNO ParseType, CNseItem** ppItem)
{
   ...
   HR( _FlickrAPI.UpdateImagesetPhotoList(m_pImageset) );
   HR( _FlickrAPI.GetPhoto(pwstrName, m_pImageset->pAccount, &pPhoto) );
...
{
   if( !IsBitSet(grfFlags, SHCONTF_NONFOLDERS|SHCONTF_STORAGE) ) 
      return S_OK;
   HR( _EnsureFlickrRef() );
   CFlickrPhotoList aList;
   HRESULT Hr = _FlickrAPI.GetImagesetPhotoList(m_pImageset, aList);
   if( HRESULT_CODE(Hr) == ERROR_NOT_FOUND ) 
      Hr = S_FALSE;
   if( FAILED(Hr) && hwndOwner != NULL ) 
      DoDisplayErrorMessage(hwndOwner, IDS_ERR_ENUM, Hr);
...
{
   HR( _EnsureFlickrRef() );
   HR( _FlickrAPI.DeleteImageset(m_pImageset) );
   HR( _RefreshFolderView() );
   return S_OK;
}
...
HRESULT CFlickrItemImageset::Refresh(VFS_REFRESH Reason)
{
   if( Reason != VFS_REFRESH_USERFORCED ) 
      return E_NOTIMPL;
   HR( _EnsureFlickrRef() );
   HR( _FlickrAPI.RefreshImagesetPhotoList(m_pImageset) );
   return S_OK;
}
...
{
   HR( _EnsureFlickrRef() );
   HR( _FlickrAPI.SetImagesetProperty(m_pImageset, PKEY_Title, CComPropVariant(pstrNewName)) );
   HR( _RefreshFolderView() );
   return S_OK;
}
...
HRESULT CFlickrItemImageset::_EnsureFlickrRef()
{
   ...
   if( !pRoot.IsType(FLICKRFS_MAGIC_ID_ACCOUNT) ) 
      return E_UNEXPECTED;
   CPidlMemPtr<FLICKRFS_PIDL_IMAGESET> pItem = m_pidlItem;
   if( !pItem.IsType(FLICKRFS_MAGIC_ID_IMAGESET) ) 
      return E_UNEXPECTED;
   TFlickrAccount* pAccount = NULL;
   HR( _FlickrAPI.FindAccountFromNSID(pRoot->cNSID, &pAccount) );
   return _FlickrAPI.FindImageset(pAccount, pItem->cImagesetID, &m_pImageset);
}
...
HRESULT CFlickrItemImageset::_DoPastePhotos(VFS_MENUCOMMAND& Cmd)
{
   ...
   else
   {
      ...
      for( DWORD i = 0; i < dwItemCount; i++ ) {
         CPidl pidlChild;
         CComPtr<IShellItem> spChild;
         HR( spItems->GetItemAt(i, &spChild) );
         HR( pidlChild.CreateFromObject(spChild) );
         CPidlMemPtr<FLICKRFS_PIDL_PHOTO> pChild = pidlChild.GetLastItem();
         if( !pChild.IsType(FLICKRFS_MAGIC_ID_PHOTO) ) 
            continue;
         TFlickrPhoto* pPhoto = NULL;
         HR( _FlickrAPI.GetPhoto(pChild->cPhotoID, m_pImageset->pAccount, &pPhoto) );
         HR( _FlickrAPI.PostProcessPhotoUpload(pPhoto, m_pImageset) );
         ::SHChangeNotify(SHCNE_UPDATEITEM, SHCNF_IDLIST, pidlChild);
      }
   }
   HR( _RefreshFolderView() );
   return Hr;
