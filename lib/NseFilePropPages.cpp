...
LRESULT CNseFileItemPropertyPage::OnInitDialog(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
   ...
   if( pNseItem->IsFolder() || dwItemCount > 1 ) {
      ...
      for( DWORD i = 0; i < dwItemCount && i < lengthof(pData->pidls); i++ ) {
         CComPtr<IShellItem> spChildItem;
         m_pShellItems->GetItemAt(i, &spChildItem);
         pData->pidls[i].CreateFromObject(spChildItem);
      }
   ...
   if( spExtractIcon != NULL ) {
      ...
      if( Hr == S_OK ) 
         Hr = spExtractIcon->Extract(wszIconFile, iIconIndex, &hIconLarge, &hIconSmall, MAKELPARAM(32, 32));
      if( Hr == S_FALSE ) 
         ::SHGetFileInfo(wszFileName, dwFileAttribs, &sfi, sizeof(sfi), SHGFI_USEFILEATTRIBUTES | SHGFI_LARGEICON), hIconLarge = sfi.hIcon;
      if( hIconSmall != NULL ) 
         ::DestroyIcon(hIconSmall);
      if( hIconLarge != NULL ) 
         SendDlgItemMessage(IDC_APPICON, STM_SETIMAGE, IMAGE_ICON, (LPARAM) hIconLarge);
   }
   // File attributes...
   if( vFileAttribs.vt == VT_UI4 && dwItemCount == 1 ) {
      if( IsBitSet(vFileAttribs.uiVal, FILE_ATTRIBUTE_READONLY) ) 
         CheckDlgButton(IDC_READONLY, BST_CHECKED);
      if( IsBitSet(vFileAttribs.uiVal, FILE_ATTRIBUTE_HIDDEN) ) 
         CheckDlgButton(IDC_HIDDEN, BST_CHECKED);
   ...
   while( aQueue.GetCount() > 0 && ::IsWindow(Info.hWnd) ) {
      ...
      if( spEnumItems != NULL ) {
         ...
         while( spEnumItems->Next(1, &spNextItem, &uFetched), uFetched > 0 ) {
            ...
            if( spShellItem2 != NULL ) 
               aQueue.Add(spShellItem2);               
            spNextItem.Release();
            uFetched = 0;
         }
      }      
      ...
      if( ::GetTickCount() - dwLastTick > PARTIAL_TIMEOUT_MS ) {
         ...
         if( ::IsWindow(Info.hWnd) ) 
            ::PostMessage(Info.hWnd, WM_FILECOUNT_UPDATE, dwTotalCount, (LPARAM) ullTotalSize);
      }
   }
