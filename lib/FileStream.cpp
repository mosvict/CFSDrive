...
STDMETHODIMP CFileStream::CopyTo(IStream* pstm, ULARGE_INTEGER cb, ULARGE_INTEGER* pcbRead, ULARGE_INTEGER* pcbWritten)
{
   ...
   if( pcbRead != NULL ) 
      pcbRead->QuadPart = size.QuadPart;
   if( pcbWritten != NULL ) 
      pcbWritten->QuadPart = size.QuadPart;
   return S_OK;
}
...
STDMETHODIMP CFileStream::Stat(STATSTG* pstatstg, DWORD grfStatFlag)
{
   ...
   if( !IsBitSet(grfStatFlag, STATFLAG_NONAME) ) 
      ::SHStrDup(wfd.cFileName, &pstatstg->pwcsName);
   ...
   if( pstatstg->cbSize.QuadPart == 0 && !IsBitSet(grfStatFlag, STATFLAG_NOOPEN) ) {
      if( SUCCEEDED(_OpenFileNow(GENERIC_READ)) ) 
         m_pStream->GetFileSize(&pstatstg->cbSize.LowPart);
   }
