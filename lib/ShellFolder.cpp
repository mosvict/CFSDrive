...
#include "pch.h"
#include "ShCFSDriveRes.h"

#include "../CFSDrive/dllmain.h"
...
HRESULT WINAPI CShellFolder::UpdateRegistry(BOOL bRegister) throw()
{
   ...
   HRESULT Hr = S_OK;
   Hr = _AtlModule.UpdateRegistryFromResource(IDR_SHELLFOLDER, bRegister, regMapEntries);
   if (SUCCEEDED(Hr)) {
   ...
   if( SUCCEEDED(Hr) && _ShellModule.GetConfigBool(VFS_INSTALL_CUSTOMSCRIPT) ) {
	  ...
	  Hr = _AtlModule.UpdateRegistryFromResource(IDR_CFSDRIVE, bRegister, regMapEntries);
   }   
...
STDMETHODIMP CShellFolder::ParseDisplayName(HWND hwnd, IBindCtx* pbc, PWSTR pszDisplayName, ULONG* pchEaten, PIDLIST_RELATIVE* ppidl, ULONG* pdwAttributes)
{
   ...
   if( wcschr(wszComponent, '\\') != NULL ) 
      *wcschr(wszComponent, '\\') = '\0';
   ...
   if( pwstrNext != NULL ) {
      ...
      if( pdwAttributes != NULL ) 
         GetAttributesOf(1, (PCUITEMID_CHILD_ARRAY) &pidl.m_p, pdwAttributes);
   }
   if( pchEaten != NULL ) 
      *pchEaten += (ULONG) wcslen(wszComponent);
   *ppidl = pidl.Detach();
   return S_OK;
}
...
STDMETHODIMP CShellFolder::CompareIDs(LPARAM lParam, PCUIDLIST_RELATIVE pidl1, PCUIDLIST_RELATIVE pidl2)
{
   ...
   if( IsBitSet(uColumnFlags, SHCIDS_ALLFIELDS) ) 
      nMaxSpanColumns = 6, bHaveUniqueNames = FALSE;
   if( IsBitSet(uColumnFlags, SHCIDS_CANONICALONLY) && bHaveUniqueNames ) 
      SortKey = PKEY_ParsingName;
   while( !::ILIsEmpty(pidl1) || !::ILIsEmpty(pidl2) ) 
   {
      ...
      if( !bHaveUniqueNames || iSortColumn != 0 ) {
         for( UINT iColumn = 0; iColumn < nMaxSpanColumns; iColumn++ ) {
            ...
            if( FAILED( m_spFolderItem->GetColumnInfo(iColumn, Column2) ) ) 
               continue;
            if( !IsBitSet(Column2.dwFlags, SHCOLSTATE_ONBYDEFAULT) ) 
               continue;
...
STDMETHODIMP CShellFolder::GetDisplayNameOf(PCUITEMID_CHILD pidl, SHGDNF uFlags, STRRET* psrName)
{
   ...
   if( uFlags == (SHGDN_FORADDRESSBAR|SHGDN_FORPARSING) ) 
      bNeedsParsingName = false;
   ...
   str += v.pwszVal;
   return StrToSTRRET(str, psrName);
}
...
STDMETHODIMP CShellFolder::GetDefaultColumn(DWORD /*dwRes*/, ULONG* pSort, ULONG* pDisplay)
{
   ...
   if( pSort != NULL ) 
      *pSort = Settings.uDefaultSortColumn;
   if( pDisplay != NULL ) 
      *pDisplay = Settings.uDefaultDisplayColumn;
   return S_OK;
}
...
STDMETHODIMP CShellFolder::GetOverlayIconIndex(PCUITEMID_CHILD pidl, int* pIconIndex)
{
   ...
   if( Hr == S_OK ) 
      *pIconIndex = INDEXTOOVERLAYMASK(iIndex);
   return Hr;
}
...
STDMETHODIMP CShellFolder::GetPaneState(REFEXPLORERPANE ep, EXPLORERPANESTATE* peps)
{
   ...
   *peps = EPS_DONTCARE;
   for( int i = 0; i < lengthof(aList); i++ ) {
      if( ep == aList[i].ep && _ShellModule.GetConfigBool(aList[i].config) ) 
         *peps = aList[i].peps;
   }
...
STDMETHODIMP CShellFolder::Stat(STATSTG* pStatstg, DWORD grfStatFlag)
{
   ...
   if( !IsBitSet(grfStatFlag, STATFLAG_NONAME) ) 
      pStatstg->pwcsName = ::StrDup(wfd.cFileName);
...
STDMETHODIMP CShellFolder::GetValidCharacters(LPWSTR* ppwszValidChars, LPWSTR* ppwszInvalidChars)
{
   ...
   if( ppwszValidChars != NULL ) 
      *ppwszValidChars = strAllowed.Detach();
   if( ppwszInvalidChars != NULL ) 
      *ppwszInvalidChars = strNotAllowed.Detach();
   return S_OK;
}
...
HRESULT CShellFolder::_ParseDisplayNameWithBind(CNseItemPtr& spItem, PWSTR pszDisplayName, IBindCtx* pbc, const BIND_OPTS& Opts)
{
   ...
   if( spItem == NULL && Opts.grfMode == STGM_CREATE ) {
      ...
      if( spFSBD != NULL ) 
         spFSBD->GetFindData(&wfd);
      if( wfd.cFileName[0] == '\0' ) 
         wcscpy_s(wfd.cFileName, lengthof(wfd.cFileName), pszDisplayName);
