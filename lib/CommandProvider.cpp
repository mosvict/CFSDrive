...
HRESULT CExplorerCommandProvider::Init(CShellFolder* pFolder, HWND hWnd, HMENU hMenu)
{
   ...
   if( !::IsMenu(hMenuPopup) || ::GetMenuItemCount(hMenuPopup) == 0 ) 
      return E_HANDLE;
...
HRESULT CExplorerCommandProvider::_GetCommand(HMENU hMenu, UINT uMenuID, REFIID riid, LPVOID* ppRetVal)
{
   ...
   if( !::GetMenuItemInfo(hMenu, uMenuID, FALSE, &mii) ) 
      return E_FAIL;
...
{
   *pCmdState = 0;
   if( IsBitSet(m_mii.fState, MFS_CHECKED) ) 
      *pCmdState |= ECS_CHECKED;
   if( IsBitSet(m_mii.fState, MFS_DISABLED) ) 
...
STDMETHODIMP CExplorerCommand::Invoke(IShellItemArray* psiItemArray, IBindCtx* pbc)
{
   ...
   if( Hr == S_FALSE ) 
      Hr = m_spProvider->m_spFolder->m_spFolderItem->ExecuteMenuCommand(Cmd);
   return Hr;
}
...
{
   *pFlags = 0;
   if( m_mii.hSubMenu != NULL ) 
      *pFlags |= ECF_HASSUBCOMMANDS;
   if( m_mii.fType == MFT_SEPARATOR ) 
