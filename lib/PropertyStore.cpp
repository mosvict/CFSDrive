...
HRESULT CPropertyStoreFactory::_GetColumnKeyMap(GETPROPERTYSTOREFLAGS gpsFlags, CSimpleValArray<PROPERTYKEY>& aKeys) const
{
   ...
   while( true ) {
      if( FAILED( m_spFolder->m_spFolderItem->GetColumnInfo(iColumn++, Info) ) ) 
         break;
      if( IsBitSet(gpsFlags, GPS_READWRITE) && !IsBitSet(Info.dwAttributes, VFS_COLF_WRITEABLE) ) 
         continue;
      if( IsBitSet(gpsFlags, GPS_FASTPROPERTIESONLY) && IsBitSet(Info.dwFlags, SHCOLSTATE_SLOW) ) 
         continue;
...
HRESULT CPropertyStoreFactory::_FilterKeyMap(GETPROPERTYSTOREFLAGS gpsFlags, const PROPERTYKEY* rgKeys, UINT cKeys, CSimpleValArray<PROPERTYKEY>& aKeys) const
{
   ...
   for( UINT i = 0; i < cKeys; i++ ) {
      ...
      while( true ) {
         ...
         if( Info.pkey != rgKeys[i] ) 
            continue;
         if( IsBitSet(gpsFlags, GPS_READWRITE) && !IsBitSet(Info.dwAttributes, VFS_COLF_WRITEABLE) ) 
            break;
         if( IsBitSet(gpsFlags, GPS_FASTPROPERTIESONLY) && IsBitSet(Info.dwFlags, SHCOLSTATE_SLOW) ) 
            break;
         if( !aKeys.Add(Info.pkey) ) 
...
#ifdef _DEBUG
   if( FAILED(Hr) || ppropvar->vt == VT_EMPTY ) 
      ATLTRACE(L"CPropertyStore::GetValue - failed\n");
#endif // _DEBUG
   return S_OK;
}
...
STDMETHODIMP CPropertyStore::IsPropertyWritable(REFPROPERTYKEY pkey)
{
   ...
   while( true ) {
      ...
      if( pkey != Info.pkey ) 
         continue;
      if( !IsBitSet(Info.dwAttributes, VFS_COLF_WRITEABLE) ) 
         return S_FALSE;
