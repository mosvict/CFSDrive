...
GUID UuidFromString(LPOLESTR pwstr)
{
   ...
   CLSID clsid = { 0 };
   if( pwstr[0] == '{') 
      ::CLSIDFromString(pwstr, &clsid);
   else 
      ::CLSIDFromProgID(pwstr, &clsid);
   return clsid;
}
...
HMENU GetSubMenu(HMENU hMenu, LPCTSTR pstrTitle)
{
   ...
   for( UINT i = 0; i < nMenuItems; i++ ) {
      ...
      if( !::GetMenuItemInfo(hMenu, i, TRUE, &mii) ) 
         return NULL;
      if( _tcscmp(szTitle, pstrTitle) == 0 ) 
         return mii.hSubMenu;
   }
   return NULL;
}
...
HRESULT SHFilterEnumItem(SHCONTF grfFlags, const WIN32_FIND_DATA& wfd)
{
   ...
   if( (grfFlags == SHCONTF_STORAGE) && IsBitSet(wfd.dwFileAttributes, FILE_ATTRIBUTE_DIRECTORY|FILE_ATTRIBUTE_VIRTUAL) ) 
      return S_OK;
   if( !IsBitSet(grfFlags, SHCONTF_FOLDERS) && IsBitSet(wfd.dwFileAttributes, FILE_ATTRIBUTE_DIRECTORY) ) 
      return S_FALSE;
   if( !IsBitSet(grfFlags, SHCONTF_NONFOLDERS) && !IsBitSet(wfd.dwFileAttributes, FILE_ATTRIBUTE_DIRECTORY) ) 
      return S_FALSE;
   if( !IsBitSet(grfFlags, SHCONTF_INCLUDEHIDDEN) && IsBitSet(wfd.dwFileAttributes, FILE_ATTRIBUTE_HIDDEN) ) 
      return S_FALSE;
   return S_OK;
}
...
HRESULT SHCreateFileOperation(HWND hWnd, DWORD dwFlags, IFileOperation** ppRetVal)
{
   ...
   if( hWnd != NULL ) 
      (*ppRetVal)->SetOwnerWindow(hWnd);
   if( dwFlags != 0 ) 
      (*ppRetVal)->SetOperationFlags(dwFlags);
   return S_OK;
}
 ...
 */
HRESULT SHCreateExtractIconFromModuleIcon(int iIconIndex, REFIID riid, LPVOID* ppRetVal)
{
   ATLASSERT(iIconIndex>=0);
   ATLASSERT(ppRetVal!=NULL);
   ATLASSERT(*ppRetVal==NULL);
...
/**
 * Create a default IExtractIcon component for a Known Folder.
 */
HRESULT SHCreateExtractIconFromKnownFolder(REFKNOWNFOLDERID folder, REFIID riid, LPVOID* ppRetVal)
{
   ATLASSERT(ppRetVal!=NULL);
   ATLASSERT(*ppRetVal==NULL);
   CPidl pidl;
   CComPtr<IShellItem> spItem;
   HR( ::SHGetKnownFolderIDList(folder, 0, NULL, (PIDLIST_ABSOLUTE*) &pidl.m_p) );
   HR( ::SHCreateItemFromIDList(pidl, IID_PPV_ARGS(&spItem)) );
   HR( spItem->BindToHandler(NULL, BHID_ThumbnailHandler, riid, ppRetVal) );
   return S_OK;
}
...
HRESULT SHGetFileSysIcon(LPCTSTR pstrFileName, DWORD dwFileAttributes, UINT uIconFlags, int* pIconIndex)
{
   ...
   if( IsBitSet(uIconFlags, GIL_OPENICON) ) 
      uFlags |= SHGFI_OPENICON;
   if( IsBitSet(uIconFlags, GIL_FORSHELL) ) 
      uFlags |= SHGFI_SHELLICONSIZE;
   if( ::SHGetFileInfo(pstrFileName, dwFileAttributes, &sfi, sizeof(sfi), uFlags) == 0 ) 
      return S_FALSE;
...
static CComVariant LoadResourceToVariant(LPCTSTR pstrName, LPCTSTR pstrType)
{
   ...
   if( hglb != NULL ) 
      ::InitVariantFromBuffer(static_cast<LPCSTR>(::LockResource(hglb)), dwLen, &vRes);
   ::FreeResource(hglb);
   return vRes;
}
...
HRESULT UpdateSendToFromProject(LPCWSTR pstrDisplayName, LPCWSTR pstrProgId, BOOL bRegister)
{
   ...
   if( bRegister ) 
      CHandle( ::CreateFileW(wszFilename, GENERIC_WRITE, NULL, 0, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL) );
   else 
...
HRESULT UpdateStartMenuLink(LPCWSTR pstrDisplayName, LPCWSTR pstrDescription, KNOWNFOLDERID FolderId, BOOL bRegister)
{
	...
	if( bRegister ) 
		Hr = spPersist->Save(wszLinkFilename, TRUE);
	else 
		::DeleteFileW(wszLinkFilename);
	return Hr;
	*/
	return S_OK;
}
...
LPCWSTR DbgGetIID(REFIID riid)
{
   ...
   if( szName[0] == '\0' ) 
      wcscpy_s(szName, lengthof(szName), CW2T(wszGUID));
...
LPCWSTR DbgGetPKEY(REFPROPERTYKEY pkey)
{
   ...
   if( wszName[0] == '\0' ) {
      ...
      key.Open(HKEY_CLASSES_ROOT, _T("Named GUIDs"), KEY_READ);
      if( key.Open(key, wszGUID, KEY_READ) == NOERROR ) 
         ::RegQueryValueExW(key.m_hKey, NULL, NULL, &dwType, (LPBYTE) wszName, &dwCount);
   }
   // Just format the GUID
   if( wszName[0] == '\0' ) 
      ::PSStringFromPropertyKey(pkey, wszName, lengthof(wszName));
...
LPCTSTR DbgGetSHGDNF(SHGDNF Flags)
{
   ...
   for( int i = 0; i < lengthof(aList); i++ ) {
      if( IsBitSet(Flags, aList[i].f) ) 
         _tcscat_s(szText, lengthof(szText), aList[i].psz);
   }
   if( szText[0] == '\0' ) 
      _tcscpy(szText, _T(" NORMAL"));
