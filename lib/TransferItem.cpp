...
void CTransferSource::FinalRelease()
{
   ...
   if( m_spFolder != NULL ) 
      m_spFolder->m_spFolderItem->TransferDone();
}
...
STDMETHODIMP CTransferSource::GetDefaultDestinationName(IShellItem* psiSource, IShellItem* psiParentDest, LPWSTR* ppszDestinationName)
{
   ...
   if( spItem == NULL ) return AtlHresultFromWin32(ERROR_FILE_NOT_FOUND);
   CComPropVariant v;
   if( SUCCEEDED( spItem->GetProperty(PKEY_FileName, v) ) ) 
      return ::SHStrDup(v.pwszVal, ppszDestinationName);
   if( SUCCEEDED( spItem->GetProperty(PKEY_ItemName, v) ) ) 
      return ::SHStrDup(v.pwszVal, ppszDestinationName);
   if( SUCCEEDED( spItem->GetProperty(PKEY_ParsingName, v) ) ) 
      return ::SHStrDup(v.pwszVal, ppszDestinationName);
...
{
   //if( _ShellModule.GetConfigBool(VFS_IGNORE_TRANSFER_NOTIFY) ) 
   //   return S_FALSE;
   if( IsBitSet(dwFlags, TSF_SUSPEND_SHELLEVENTS) ) 
      return S_FALSE;
   if( pItem1->IsFolder() ) 
   {
      switch( wEventId ) {
...
STDMETHODIMP CTransferDestination::CreateItem(LPCWSTR pszName, DWORD dwAttributes, ULONGLONG ullSize, DWORD dwFlags, REFIID riidItem, LPVOID* ppvItem, REFIID riidResources, LPVOID* ppvResources)
{
   ...
   if( riidItem == IID_IShellItem || riidItem == IID_IShellItem2 )
   {
      ...
      if( spPrevItem != NULL ) {
         ...
         if( !bIsFolder ) {
            if( !IsBitSet(dwFlags, TSF_OVERWRITE_EXIST) ) 
...
STDMETHODIMP CShellItemResources::GetTimes(FILETIME* pftCreation, FILETIME* pftWrite, FILETIME* pftAccess)
{
   ...
   if( pftCreation != NULL ) 
      *pftCreation = wfd.ftCreationTime;
   if( pftAccess != NULL ) 
      *pftAccess = wfd.ftLastAccessTime;
   if( pftWrite != NULL ) 
      *pftWrite = wfd.ftLastAccessTime;
   return S_OK;
}
...
STDMETHODIMP CShellItemResources::SetTimes(const FILETIME* pftCreation, const FILETIME* pftWrite, const FILETIME* pftAccess)
{
   ...
   if( pftCreation != NULL ) 
      m_spItem->SetProperty(PKEY_DateCreated, *pftCreation);
   if( pftAccess != NULL ) 
      m_spItem->SetProperty(PKEY_DateAccessed, *pftAccess);
   if( pftWrite != NULL ) 
      m_spItem->SetProperty(PKEY_DateModified, *pftWrite);
   return S_OK;
}
...
{
   ATLTRACE(L"CShellItemResources::MarkForDelete");
   if( m_spItem == NULL ) return E_NOTIMPL;
   return m_spItem->MarkForDelete();
}
