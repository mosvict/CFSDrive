...
HRESULT CNseFileItem::GetProperty(REFPROPERTYKEY pkey, CComPropVariant& v)
{
   ...
   if( pkey == PKEY_DescriptionID ) {
      SHDESCRIPTIONID did = { (DWORD)(IsFolder() ? SHDID_FS_DIRECTORY : SHDID_FS_FILE), CLSID_ShellFolder };
...
{
   SFGAOF Flags = 0;
   if( IsBitSet(m_pWfd->dwFileAttributes, FILE_ATTRIBUTE_HIDDEN) ) 
      Flags |= SFGAO_HIDDEN;
   if( IsBitSet(m_pWfd->dwFileAttributes, FILE_ATTRIBUTE_READONLY) ) 
      Flags |= SFGAO_READONLY;
   if( IsBitSet(m_pWfd->dwFileAttributes, FILE_ATTRIBUTE_DIRECTORY) ) 
      Flags |= SFGAO_DROPTARGET;
   if( IsBitSet(m_pWfd->dwFileAttributes, FILE_ATTRIBUTE_DIRECTORY) ) 
      Flags |= SFGAO_FOLDER | SFGAO_HASSUBFOLDER | SFGAO_BROWSABLE | SFGAO_FILESYSANCESTOR;
   if( IsBitSet(m_pWfd->dwFileAttributes, FILE_ATTRIBUTE_REPARSE_POINT) ) 
      Flags |= SFGAO_LINK;
   // We bump to FILESYSTEM mode when entering the SaveAs hack
   if( m_pWfd->cAlternateFileName[1] == VFS_HACK_SAVEAS_JUNCTION ) 
      Flags |= SFGAO_FILESYSTEM;
   // We support IStream for files and IStorage for folders
   if( IsBitSet(m_pWfd->dwFileAttributes, FILE_ATTRIBUTE_DIRECTORY) ) 
      Flags |=  SFGAO_STORAGE | SFGAO_STORAGEANCESTOR;
   else 
      Flags |= SFGAO_STREAM;
   return Flags;
}
